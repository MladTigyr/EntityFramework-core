...
namespace TravelAgency.Data
{
    ...
    {
        private const string connectionString = @"Server=DESKTOP-B752TI8\SQLEXPRESS;Database=TravelAgency;Integrated Security=True";
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<Customer>()
                .HasCheckConstraint("CK_Customer_FullName_Length",
                $"LEN([FullName]) >= {MinCustomerFullNameLength} AND LEN([FullName]) <= {MaxCustomerFullNameLength}");

            modelBuilder.Entity<Customer>()
                .HasCheckConstraint("CK_Customer_Email_Length",
                $"LEN([Email]) >= {MinCustomerEmailLength} AND LEN([Email]) <= {MaxCustomerEmailLength}");
            ...
            modelBuilder.Entity<Guide>()
                .HasCheckConstraint("CK_Guide_FullName_Length",
                $"LEN([FullName]) >= {MinGuideFullNameLength} AND LEN([FullName]) <= {MaxGuideFullNameLength}");

            modelBuilder.Entity<TourPackage>()
                .HasCheckConstraint("CK_PackageName_Length",
                $"LEN([PackageName]) >= {MinPackageNameLength} AND LEN([PackageName]) <= {MaxPackageNameLength}");

            modelBuilder.Entity<TourPackage>()
                .HasCheckConstraint("CK_TourPackage_Price",
                "[Price] >= 0");
            ...
            modelBuilder.Entity<TourPackageGuide>(entity =>
            {
                ...
                entity
                .HasOne(tg => tg.TourPackage)
                .WithMany(t => t.TourPackagesGuides)
                .HasForeignKey(tg => tg.TourPackageId)
                .OnDelete(DeleteBehavior.Restrict);
            });
